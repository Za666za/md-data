...
### 1.4 当前主流框架及技术定位
*   **Servlet规范**：底层标准，定义了如何接收HTTP请求，需Tomcat等容器实现。它是所有Java Web框架的**地基**（就像国家的交通法规）。
*   **Spring**：为解决企业应用程序开发复杂性而创建的开源框架，核心是分层架构与依赖注入。它是Java开发的**核心指挥官**。
*   **Spring MVC**：基于MVC设计模式实现的Web开发框架，作为控制器实现模型与视图的数据交互。**它封装了原生Servlet，提供了DispatcherServlet作为前端控制器**。
*   **MyBatis**：优秀的持久层框架，支持普通SQL查询、存储过程及高级映射，是半自动化的ORM实现。它是**“档案员”**，负责SQL执行和数据映射。
*   **Spring Boot**：致力于简化Spring配置、提升开发体验的微框架，是未来发展的趋势。它是**“全自动房车”**（内嵌Tomcat + 自动配置）。
### 1.5 【新增】MVC架构模式与前后端分离演进
*   **传统MVC时代**：Model(数据)、View(JSP页面)、Controller(Servlet)全部在后端。后端负责渲染HTML页面发给浏览器。Servlet 早期甚至需要通过字符串拼接生成 HTML（`out.println("<html>...")`）。
*   **现代前后端分离时代**：
    *   **View的剥离**：View不再由后端渲染，而是交由前端框架（如React/Vue）。前端编译生成的JS/CSS等静态文件就是最终的View。
    *   **React与Nginx的关系**：React 代码（JSX）经过脚手架（如 Vite/CRA）打包（`npm run build`）变成纯静态文件（HTML/JS），由 Nginx 这种高效的**Web服务器**提供访问。Nginx 扮演**“文件仓库管理员”**的角色，负责物理映射（URL 对应磁盘文件）。
    *   **Spring Boot的角色**：退化为纯粹的API数据提供者。它运行在 Tomcat 这种**应用服务器**中，扮演**“加工车间”**的角色，负责执行 Java 代码，生成 JSON 数据。前端通过 Ajax 请求获取后端返回的 JSON 数据，在浏览器端自行渲染页面。
---
## 2. Spring框架基础
### 2.1 Spring之父与设计理念
...
*   **角色定位**：MyBatis 是具体的**执行者（工具/锤子）**，负责执行 SQL。但它是被 Spring（包工头）调度的。
...
*   **Servlet的映射规则**：Servlet 的映射取决于**代码逻辑**（`@WebServlet` 或 `web.xml`），它是**虚拟**的，与物理文件结构无关。
*   **Spring MVC的解法**：提供一个核心的`DispatcherServlet`（前端控制器）拦截所有请求，通过“注解化开发”将URL直接映射到普通Java方法上。它本质依然是一个 Servlet。
...
*   **内嵌Tomcat机制**：在传统JSP时代，需外部安装Tomcat并部署war包；在Spring Boot中，Tomcat被封装为一个普通的jar包依赖（`tomcat-embed-core`）。当执行`main`方法时，Java程序先启动，然后在内部`new`一个Tomcat对象并启动它。
*   **Spring Boot 公式**：Spring Boot ≈ Spring MVC + 内嵌 Tomcat + 自动配置。
...
1.  **底层规范**：Servlet是Java Web的地基，所有上层框架均基于此构建。它虽然隐藏在框架之后，但其**逻辑映射**的特性决定了Java Web的本质。
2.  **Spring框架**是基石，其核心的IoC和AOP思想解决了组件耦合问题，规范了企业级开发。它像**“包工头”**一样调度所有资源。
3.  **MyBatis框架**作为持久层方案，灵活高效地处理了Java对象与关系型数据库之间的映射，化解了JDBC的硬编码痛点。它是**“档案员”**，负责具体的SQL执行。
4.  **Spring MVC框架**是Web层的核心，通过DispatcherServlet和注解化开发，彻底封装了原生Servlet的繁琐操作，实现了高效的请求分发。
5.  **Spring Boot框架**是效率倍增器，通过“约定优于配置”和内嵌Tomcat的理念，极大地简化了Spring应用的搭建与部署。
6.  **前后端协作新范式**：在现代化架构中，前端（React+JSX）打包后由 **Nginx（文件仓库）** 提供静态服务，后端由 Spring Boot + 内嵌 Tomcat（加工车间）提供 API 接口服务，两者通过 JSON 数据进行高效协作。
